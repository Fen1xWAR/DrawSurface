...
def load_fdf_file(filename: str) -> np.ndarray:
    ...
    for line in lines:
        ...
        for item in line.split():  # Разделить строку на элементы
            if ',' in item:  # Если элемент содержит запятую (высота и цвет)
                ...
                row.append(np.array([height, color], dtype=object))  # Добавление кортежа (высота, цвет) в строку
...
def create_vertices_and_colors(matrix: np.ndarray, scale_z: float = 0.1) -> tuple:
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if isinstance(matrix[i, j], np.ndarray):  # Если элемент - кортеж (высота, цвет)
                z, color = matrix[i, j]  # Извлечение высоты и цвета
                z = float(z) * scale_z  # Масштабирование высоты
                color = convert_hex_to_rgb(color)  # Преобразование цвета в RGB
            else:  # Если только высота
                z = matrix[i, j] * scale_z  # Масштабирование высоты
                color = (1.0, 1.0, 1.0)  # Цвет по умолчанию - белый

            vertices.append((x, y, z))  # Добавление вершины в список
...
def create_edges(rows: int, cols: int) -> np.ndarray:
...
def draw_plane(vertices: np.ndarray, colors: np.ndarray, edges: np.ndarray):
    ...
    index_buffer = glGenBuffers(1)  # Создание буфера для индексов
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index_buffer)  # Привязка буфера индексов
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, edges.nbytes, edges, GL_STATIC_DRAW)  # Заполнение буфера индексов
...
def main_program(matrix: np.ndarray):
    ...
    for row in matrix:
        for v in row:
            heights.append(float(v[0]) if isinstance(v, np.ndarray) else float(v))  # Сбор высот в список
    ...
    zoom = -10  # Начальное значение зума
    ...
    while True:  # Основной цикл программы
        for event in pygame.event.get():  # Обработка событий pygame
            if event.type == pygame.QUIT:  # Проверка на выход
                ...
                zoom += event.y  # Изменение значения зума
...
if __name__ == "__main__":
    matrix = load_fdf_file('test_maps/nerd_colored.fdf')  # Замените на путь к вашему файлу
