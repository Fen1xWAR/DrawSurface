...
def parse_without_color(image):
    ...
    for row in grayscale_values:
        parsed_row = [f"{int(val)},{color_map[0 if val == 0 else (1 if val > 0 else -1)]}" for val in row]
...
def main():
    ...
    try:
        ...
        avg_matrix = np.mean(image_matrix, axis=2)  # Среднее значение яркости пикселя
        height_matrix = np.round(np.log(avg_matrix + 1)).astype(int)  # Применяем логарифм и округляем

        # Извлекаем цвета из каждого пикселя
        colors_matrix = image_matrix[:, :, :3]  # RGB каналы
        hex_colors = np.apply_along_axis(lambda rgb: f'0x{rgb[0]:02x}{rgb[1]:02x}{rgb[2]:02x}', 2, colors_matrix)

        # Создаём карту рельефа
        relief_map = [[f'{h},{c}' for h, c in zip(height_row, color_row)] for height_row, color_row in
                      zip(height_matrix, hex_colors)]

    except Exception as e:
        print(f'\033[31mError processing image: {str(e)}. Using default color scheme.\033[m')
